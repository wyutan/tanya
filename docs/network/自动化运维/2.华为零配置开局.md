...
::: tip
lswnet.cfg文件内容中必须包含设备唯一标识，mac或esn都可，此处为esn；对于无升级需求的，版本、补丁可以不配置；交换机的配置文件命名使用的是esn_ip.cfg格式
:::

文件内容如下：
```
esn=xxxx;vrpfile=s57li_easy_V200R022C00.cc;vrpver=V200R022C00SPC100;patchfile=s57li_easy_V200R022C00.pat;cfgfile=esn_ip.cfg;
```
...
::: tip
  通过shell脚本监控cfg目录下文件变化，并自动更新lswnet.cfg文件
:::
...
::: tip
此脚本可实现监控cfg目录的内容变化
从esn_ip.cfg文件名中提取esn及IP并更新lswnet.cfg内容
脚本名：monitor_cfg.sh
:::
...
::: tip
  脚本可根据excel中内容自动生成配置文件并上传至sftp服务器，交换机接口、IP等信息均使用变量，变量名与excel中一致，可视情况更改模板配置
:::
...
|---------|----------------|-----------|---------|---------|-------------|--------------|------|------|
| esn     | all_vlan       | vlanid    | mgmt_vlan | mgmt_ip | netmask     | gateway      |   username   |  password    |
| 111     | 121 131        | 131       | 131     | 10.1.1.2 | 255.255.255.0 | 10.1.1.1   |      |      |
### 配置模板
==文件名“初始化.cfg”=={dangerous}
``` cfg
sysname {mgmt_ip}
