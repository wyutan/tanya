...
## 配置文件服务器
并在文件服务器工作目录下创建文件`lswnet.cfg` [+lswnet]

[+lswnet]:
  版本、补丁可以不配置，必须有设备唯一标识，mac或esn都可，此处为esn，配置文件命名生产环境配合脚本使用的是esn_ip.cfg
文件内容如下：
```
...
[+DHCP]:
  告知客户端sftp服务器用户名、密码以及配置文件，配置文件中包含设备唯一标识及对应该设备的配置
```
...
[+更新]:
  通过脚本监控cfg目录下文件变化，并自动更新lswnet.cfg文件
- 配置文件名格式为`esn_ip.cfg`
...
## 初始化并上传至sftp脚本 [+脚本]
[+脚本]:
  脚本可根据excel中内容自动生成配置文件并上传至sftp服务器，交换机接口、IP等信息均使用变量，可视情况更改
### excel文件
文件名“info.xlsx”
