...
.about-layout-container {
  display: flex;
  flex-direction: column-reverse; /* 手机上头像在最上面 */
  gap: 40px;
  margin-top: 20px;
}

/* 2. 关键修改：大屏布局 (>1200px) */
/* 使用 fixed 定位，让头像脱离文档流，直接飞到右侧红框位置 */
/* 左侧内容(.about-main-col) 保持原位不动 */
@media (min-width: 1200px) {
  .about-layout-container {
    display: block; /* 恢复块级布局，让左侧内容自然居中 */
  }

  .about-sidebar-col {
    position: fixed;  /* 关键：固定定位 */
    top: 100px;       /* 距离顶部距离 */
    right: 40px;      /* 距离右侧屏幕边缘的距离 (贴近你的红框) */
    width: 260px;     /* 卡片宽度 */
    z-index: 100;     /* 保证在最上层 */
  }
